class ProgramPresenter < ModelPresenter
  ...
  def apply_or_cancel_button
    if entry = object.entries.find_by(customer_id: current_customer.id)      #エントリーしている状態
      ...
      status = program_status                                                #エントリーをまだしてない状態
      button_to button_label_text(status), [ :customer, object, :entry ],
        disabled: status != :available, method: :post,                        #プログラムが:closedまたは:fullの場合にボタンを無効
  ...
  private def cancellation_status(entry)
    if object.application_end_time.try(:<, Time.current)           #nilでない場合にその時間が現在の時間より前かどうかを確認
      :closed
