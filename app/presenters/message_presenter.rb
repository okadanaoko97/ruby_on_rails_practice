class MessagePresenter < ModelPresenter
  ...
  def truncated_subject
    view_context.truncate(subject, length: 20)      #文字列を切り捨てる
  end

  def formatted_body
    ERB::Util.html_escape(body).gsub(/\n/, "<br>").html_safe         #bodyという変数にテキストが含まれており、bodyの内容がHTMLエスケープされます。
  end                                                                 #エスケープされたテキスト内の改行文字が<br>に変換。文字列がHTMLとして安全であるとマークされ、そのまま表示されます。
